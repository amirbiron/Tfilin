...
class TefillinScheduler:
    ...
    async def check_daily_reminders(self):
        ...
        users_to_remind = self.users_collection.find({
            ...
            "$expr": {
                "$and": [
                    {"$eq": [{"$hour": {"$dateFromString": {"dateString": {"$concat": ["2000-01-01T", "$daily_time", ":00"]}}}}, current_time.hour]},
                    {"$eq": [{"$minute": {"$dateFromString": {"dateString": {"$concat": ["2000-01-01T", "$daily_time", ":00"]}}}}, current_time.minute]}
                ]
            }
        })
        ...
        for user in users_to_remind:
            ...
            self.users_collection.update_one(
                ...
                {"$set": {"last_reminder_date": current_date.isoformat()}}
            )
    ...
    async def check_sunset_reminders(self):
        ...
        users_with_sunset = self.users_collection.find({
            "active": True,
            "sunset_reminder": {"$exists": True, "$ne": 0}
        })
        ...
        for user in users_with_sunset:
            ...
            if time_diff <= 120:  # 2 דקות
                ...
                self.users_collection.update_one(
                    {"user_id": user['user_id']},
                    {"$set": {"last_sunset_reminder_date": current_date.isoformat()}}
                )
    ...
    async def send_daily_reminder(self, user_id: int):
        ...
        try:
            await self.bot_app.bot.send_message(
                chat_id=user_id,
                text=Config.MESSAGES['daily_reminder'],
            ...
            if "bot was blocked" in str(e).lower():
                self.users_collection.update_one(
                    {"user_id": user_id},
                    {"$set": {"active": False, "blocked_date": datetime.now().isoformat()}}
                )
